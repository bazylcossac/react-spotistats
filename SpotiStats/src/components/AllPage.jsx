...
const AllPage = () => {
  const [topArtists, topTracks, loading, userPlaylists] = useOutletContext()
  console.log(topArtists);
    ...
    const artistElements = topArtists?.map(artist => {
        return (
            ...
            <Link to={artist.id}>
                <img src={artist.images[2].url} alt={artist.name} className="rounded-full shadow-lg" loading="lazy"/>
            </Link>
            </div>
        );
      });
    ...
    const tracksElement = topTracks?.map((track, index) => {
            ...
            <div key={track.id} className="track px-4 py-2 my-2 mx-2 bg-[#252525] rounded-md shadow-lg">
                <div className="flex flex-row justify-between items-center">
                    <div className="flex flex-row items-center">
                        ...
                        <Link to="/">
                            <img src={track.album.images[2].url} alt={track.name} className="rounded-lg shadow-md ml-4" loading="lazy" />
                        </Link>
                       <Link to={track.album.artists[0].id}> <div className="ml-4 h-full break-words overflow-auto w-56">
      ...
      const playlistsElement = userPlaylists.map(playlist => {
        return (
          <div key={playlist.id} className=" bg-[#252525] flex flex-col rounded-lg shadow-lg  ">
            <div className="flex flex-row items-start">
            <img src={playlist.images[0].url} className="size-20 shrink-0 rounded-lg p-2 " loading="lazy"/>
            <p className="someWhite text-center ml-2 mt-1 font-bold">Total: {playlist.tracks.total}</p>
            </div>
           <p className="someWhite font-bold my-2 overflow-x-auto text-center">{playlist.name}</p> 
          </div>
        )
      })
